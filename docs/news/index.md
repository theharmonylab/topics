...
- **[`topicsTutorialData()`](https://r-topics.org/reference/topicsTutorialData.md)**:
  New utility function to download and prepare long-text essay data
  directly from Hugging Face. Supports custom `sample_size`,
  `min_word_count`, `max_word_count`, and `seed`.
- **[`topicsPlotOverview()`](https://r-topics.org/reference/topicsPlotOverview.md)**:
  Introduced a high-level plotting function for structured overviews.
  Supports side-by-side comparisons (ngrams), 1D layouts, and 2-D 3x3
  grids with a central distribution plot.

#### Improvements to `topicsTest()`

- **Categorical Variable Support**: `x_variable` and `y_variable` now
  fully support **Factors** and **Character** vectors.
- **Intelligent Method Detection**: The `test_method` is now assigned
  per-variable. The package automatically detects binary data (0/1 or
  2-level factors) to apply `logistic_regression` while using
  `linear_regression` for continuous data.
- **Baseline Reporting**: Logistic regression results now include a
  `logistic_level` string in the output list to clarify the Baseline (0)
  vs. Target (1) mapping.

#### Enhancements & Aliases

- **Function Aliasing**:
  [`topicsPreds()`](https://r-topics.org/reference/topicsPredict.md) can
  now be accessed via descriptive aliases:
  - [`topicsPredict()`](https://r-topics.org/reference/topicsPredict.md)
  - [`topicsAssess()`](https://r-topics.org/reference/topicsPredict.md)
  - [`topicsClassify()`](https://r-topics.org/reference/topicsPredict.md)
- **Visual Refinement**: Updated default color palettes in
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md) for
...
- [`topicsGrams()`](https://r-topics.org/reference/topicsGrams.md) now
  uses exact word boundary matching for n-grams (e.g., “lack” is matched
  as a standalone word, excluding partial matches like “black” or
  “lacking”).
- added ability to handle NAs in
  [`topicsTest()`](https://r-topics.org/reference/topicsTest.md).
...
- addting `scatter_legend_dots_alpha` and `scatter_legend_bg_dots_alpha`
  parameters for the
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md)
  function.
...
- added `occurance_rate` to
  [`topicsGrams()`](https://r-topics.org/reference/topicsGrams.md)
- added `removal_mode`, `removal_rate_most` and `removal_rate_least` to
  [`topicsGrams()`](https://r-topics.org/reference/topicsGrams.md)
- `ngram_window = c(1)` now supported by
  [`topicsDtm()`](https://r-topics.org/reference/topicsDtm.md)
- legend added to
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md) with
  ngrams
- The `size` in the dot legend will be based on `prevalence` if
  scatter_legend_dot_size = “prevalence”. And the popouts are not
  transparent.
...
- The default value of `highlight_topic_words` is set to `NULL` in the
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md)
  function.
...
- changed some behaviours in
  [`topicsGrams()`](https://r-topics.org/reference/topicsGrams.md),
  including removing `top_n` and treating n-grams type differently.
- added `stopwords` function to
  [`topicsGrams()`](https://r-topics.org/reference/topicsGrams.md).
...
- adding `allowed_word_overlap` in
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md) for
  plotting the most prevalence.
- improving help texts
- `highlight_topic_words` parameter to add different colours for a word
  list.
- added `stopwords` removal for `topicsGram()`.
- added `ngrams_max` functionality to
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md).

## topics 0.40.

- removing `save_dir` and `load_dir` from all function; only
  [`topicsPlot()`](https://r-topics.org/reference/topicsPlot.md) now has
  the `save_dir` as an option.
...
- scaling controls with scale instead of manually resulting in slightly
  different estimates. (but still same p-value and t-values)
- removed ridge regression, t-test and correlation codes since they did
  not work
- removed automatic removal of NAs in the topics predictions (this
  should be handled explicitly).
- topicsTest() `default` to linear_regression if not the variable only
  contains 0s and 1s; i.e., now different tests can be applied to
  different axes.

## topics 0.30.3

- saving settings in `dtm` for downstream use in other functions.
- adding parameters in the `topicsPred()` function including
  `num_iteration`, `sampling_interval`, `burn_in`.
- implemented `create_new_dtm` for creating a new `dtm` for new data
- adding test for using `topics` dimension for training using
  `textTrainRegression()`.
- removing forcing user to set save_dir on most functions (only need to
  do it for topics functions).
...
- Harmonizing parameters in
  [`topicsTest()`](https://r-topics.org/reference/topicsTest.md)
  incl. x_variable, y_variable and controls
- fixing error that variable names cannot be names with 1 underscore.

## topics 0.22.1

- added `pmi_threshold` (experimental) to
  [`topicsDtm()`](https://r-topics.org/reference/topicsDtm.md)
- removed the saving of raw data and the `split` procedure in the
  [`topicsDtm()`](https://r-topics.org/reference/topicsDtm.md)
...
- moved `p_alpha` from the
  [`topicsTest()`](https://r-topics.org/reference/topicsTest.md)
  function to the `topicsPlots()` function
- removed unnecessary list items from
  [`topicsTest()`](https://r-topics.org/reference/topicsTest.md)
