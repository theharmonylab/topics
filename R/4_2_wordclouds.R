...
#' @noRd
save_plot <- function(plot, stub, header='', save_dir, seed){
...
create_plots <- function(
  ...
  if(!is.null(df_list) && !is.null(summary) && !is.null(test)){
    ...
    for(topic in plot_topics_idx){
      ...
      if(!is.nan(p_val_min) && (p_val_min < cur_alpha || is_pop)){
        ...
        save_plot(plot, stub, 't_', save_dir, seed)
        plots[[topic]] <- plot
      }
    }
  }
  ...
  if(!is.null(df_list) && !is.null(summary) && is.null(test)){
    for(topic in plot_topics_idx){
      ...
      save_plot(plot, topic, 't_', save_dir, seed)
  ...
  if(is.null(df_list) && !is.null(ngrams)){
    if(is.null(test)){
      plot <- ggplot2::ggplot(ngrams, ggplot2::aes(label = ngrams, size = prop, color = prop)) +
        ...
        ggplot2::theme_minimal() +
        color_positive_cor
      save_plot(plot, "ngrams", '', save_dir, seed)
      plots <- plot
    } else {
      ...
      if(nrow(tst) == 0){
        ...
        save_plot(pos, "ngrams_positive", '', save_dir, seed)
        save_plot(neg, "ngrams_negative", '', save_dir, seed)
